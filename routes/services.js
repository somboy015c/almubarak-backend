...
const { applyMarkup, publicUser } = require('../utils/helpers');
...
router.get('/data/plans/:network', requireAuth, async (req, res) => {
  ...
  const priced = plans.map((p) => ({ ...p, price: applyMarkup(p.amount, pricing.data.markupPercent) }));
...
router.get('/cable/plans/:provider', requireAuth, async (req, res) => {
  ...
  const pricing = await getOrCreatePricing();
  const priced = plans.map((p) => ({ ...p, price: applyMarkup(p.amount, pricing.cable.markupPercent) }));
  res.json({ plans: priced });
});
...
router.get('/exam/plans', requireAuth, async (req, res) => {
  ...
  const pricing = await getOrCreatePricing();
  const priced = plans.map((p) => ({ ...p, price: p.amount + pricing.exam.flatFee }));
  res.json({ plans: priced });
});
...
async function chargeAndPurchase({ req, res, kind, amount, description, payload }) {
  ...
  if (!result.ok) {
    user.walletBalance = currentBalance; // refund
    await user.save();
